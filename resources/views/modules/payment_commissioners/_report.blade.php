...
<div class="main">
	...
	<div class="code-container-right">
		<span class="text-bold">#PI-{{ $paymentCommissioner->payment_code }}</span>
	</div>
	...
	<div class="body-note mt-4 mb-4">
		Por la presente, se emite el siguiente reporte de pago de comisión a favor del comisionista <strong>{{ $paymentCommissioner->commissioner->commissioner_name }}</strong>, identificado con su número de identidad 
		<strong>{{ $paymentCommissioner->commissioner->commissioner_dni }}</strong>, número de teléfono <strong>{{ $paymentCommissioner->commissioner->commissioner_phone }}</strong>, con un valor total de Lps. 
        <strong>{{ number_format($paymentCommissioner->payment_amount, 2) }}</strong> por concepto del proyecto llamado <strong>{{ $paymentCommissioner->project->project_name }}</strong> con fecha de inicio el día <strong>{{ $project->project_start_date }}</strong> y fecha de finalización <strong>{{ $project->project_end_date }}</strong>.
		<br>
		<br>
		<br>
	</div>
